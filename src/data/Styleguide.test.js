describe('service sc5.styleguide.data.Styleguide', function() {
  ...
  var service,
    ...
    rootScope,
    socketService,
    debounce = sinon.stub().callsArg(1).returnsArg(1),
    json = {
      config: {
        foo: 'bar'
      },
  ...
  beforeEach(angular.mock.module('sc5.styleguide.data'));
  ...
  it('GETs styleguide.json from .jsonUrl property on .get()', function() {
    var url = 'foo/bar.json';
    http.expectGET(url);
    http.whenGET(url).respond(json);
    service.jsonUrl = url;
    service.get();
    http.flush();
  ...
  it('GETs styleguide.json on $rootScope broadcast event "styles changed"', function() {
    http.flush();
  ...
  it('GETs styleguide.json on $rootScope broadcast event "progress end"', function() {
    http.flush();
    http.resetExpectations();
    http.expectGET('styleguide.json');
